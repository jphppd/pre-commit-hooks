...
def main(argv=None):
    ...
    parser.add_argument(
        '--css-files-dir', required=True, help='Root directory for CSS files to check'
    )
    parser.add_argument(
        '--html-files-dir', required=True, help='Root directory for HTML files to check'
    )
    parser.add_argument(
        '--ignored-missing-class-defs-pattern', help='Regular expression matching CSS class names'
    )
    parser.add_argument(
        '--ignored-unused-class-defs-pattern', help='Regular expression matching CSS class names'
    )
    ...
    css_classes_used = sum(
        [list(extract_css_classes_usages(html_file)) for html_file in html_files], []
    )
    print('Found {} CSS classes usages in HTML files'.format(len(css_classes_used)))
    css_classes_defined = sum(
        [list(extract_css_classes_definitions(css_file)) for css_file in css_files], []
    )
    ...
    if args.ignored_unused_class_defs_pattern:
        unused_css_classes = sorted(
            css_class for css_class in unused_css_classes
    ...
    if args.ignored_missing_class_defs_pattern:
        missing_css_classes = sorted(
            css_class for css_class in missing_css_classes
...
def scandir(dirname, file_extensions):
    for dirpath, _, fnames in os.walk(dirname):
        for file_path in [
            ...
            for fname in fnames if any(fname.endswith(ext) for ext in file_extensions)
        ]:
            yield file_path
